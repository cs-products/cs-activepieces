...
export const dataupdate = createAction({
  ...
  name: 'dataupdate',
  displayName: 'Mews Webhook Data Fetch',
  description: '',
  ...
  async run(context) {
    ...
    interface ApiData {
      ...
      ResourceCategoryAssignments?: any[]; // Replace with exact type if available
    }
    ...
    // Helper function to create HttpRequest objects
    ...
    const resourceCategoriesIds = new Set<string>();
    ...
    await sleep(1000);
    // try {
    //   responses['servicesRes'] = await httpClient.sendRequest<{
    //     Services: Service[];
    //   }>(createHttpPostRequest(endpoints.services));
    // } catch (error) {
    ...
    // Add additional data for further requests

    // requests.getPricing.body['ResourceIds'] = Array.from(resourcesIds);
    // Execute the second set of requests
    try {
    ...
    //     Array.from(requestedResourceCategoryIds);
    //   requests.resourceCategoryAssignments.body['ActivityStates'] = [
    //     'Active',
    //   ];
    //   responses['rcaRes'] = await httpClient.sendRequest<{
    //     ResourceCategoryAssignments: any[];
    ...
    try {
      reservations.forEach((res: Reservation) => {
        ...
        // res.RateId = ratesObj[res.RateId] || {};
      });
    } catch (error) {
      return { error };
    }
