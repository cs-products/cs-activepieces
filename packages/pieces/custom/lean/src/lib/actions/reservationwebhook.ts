...
import { createGimmyPayloadFromRequestBody, fetchRoomId } from '../../common/commonFunctions';
import { getAuthToken } from '../../common/getAuthToken';
import { getAllRooms } from '../../common/getAllRooms';

export const reservationwebhook = createAction({
  // auth: check https://www.activepieces.com/docs/developers/piece-reference/authentication,
  name: 'reservationwebhook',
  displayName: 'reservationWebhook',
  description: 'reservation updates webhook',
    props: {
  ...
  async run(context) {
    ...
    try {
      const authTokenResponse = await getAuthToken(origin, "api_clicsoft", "Clicsoft@123")
