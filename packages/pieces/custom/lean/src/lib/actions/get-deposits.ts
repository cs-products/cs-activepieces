...
export const getDeposits = createAction({
  ...
  async run(context) {
    const { body } = context.propsValue;
        ...
        if (login && login?.body?.token) {
          ...
          const transformedRes = response.body.results.map((deposit:any) => {
            return {
              yourRefId: deposit.number || null,
              folioId: deposit.reservation || null,
              paymentDate: deposit.date,
              modeOfPayment: null,
              amountInclusiveTaxes: deposit.value,
              description: null,
            };
          });
