...
export const getProducts = createAction({
  ...
  description: 'To get list of products in a hotel',
  props: {
  ...
  async run(context) {
    ...
    if (login && login?.body?.token) {
      ...
      const endpoint = `${data?.url}/api/v2/products?hotel=${Number(data?.hotelId)}`;
      ...
      const transformedRes = response.body.map((product:any) => {
        return {
          productCode: product.id || null,
          productLabel: product.name || null,
          productShortLabel: product?.shortLabel || null,
          accountingCategory: {
            classification: product.description || null,
            name: product.category_name || null,
            code: product.substatistictype || null,
          },
          price: product.price,
          tax: {
            taxCode: product.tax_id || null,
            taxValue: product.tax || null,
          },
          isActive: product.active,
        };
      });
      return {products: transformedRes};
    }
    return {
      status: 401,
