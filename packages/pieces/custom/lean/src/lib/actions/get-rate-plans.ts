...
export const getRatePlans = createAction({
  // auth: check https://www.activepieces.com/docs/developers/piece-reference/authentication,
  name: 'getRatePlans',
  displayName: 'Get Rate Plans',
  description: 'To retrieve rate plans for a hotel',
  props: {
  ...
  async run(context) {
    ...
    if (login && login?.body?.token) {
      ...
      const endpoint = `${data?.url}/api/v2/rates?hotel=${Number(data?.hotelId)}`;
      ...
      // return response;
      const transformedRes = response.body.results.map((rate:any) => {
        return {
          rateCode: rate.code || null,
          rateLabel: rate.description.en || null,
          productShortLabel: rate?.shortLabel || null,
          priceType: rate.refund,
          isActive: true,
          isEnabled: true,
          isPublic: true,
        };
      });
      return {rates: transformedRes};
    }
    return {
      status: 401,
