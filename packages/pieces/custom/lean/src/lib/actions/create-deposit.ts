...
import { ADD_DEPOSIT_OPTIONAL_PARAMS, ADD_DEPOSIT_REQUIRED_PARAMS } from '../common/constants';
  ...
  async run(context) {
    const { body } = context.propsValue;
        ...
        if (login && login?.body?.token) {
          ...
          const endpoint = `${data?.url}/api/v2/deposits/`;
          const headers: HttpHeaders = {
            Authorization: `Token ${token}`,
          };

          const transformedLeanRequestBody = transformRequest(leanBody, ADD_DEPOSIT_REQUIRED_PARAMS, ADD_DEPOSIT_OPTIONAL_PARAMS
          );
    
          transformedLeanRequestBody['value'] = parseFloat(transformedLeanRequestBody['value']); 
          transformedLeanRequestBody['reservation'] = Number(transformedLeanRequestBody['reservation']); 
          transformedLeanRequestBody['payment_type_id'] = Number(
            transformedLeanRequestBody['payment_type_id']
          ); 
