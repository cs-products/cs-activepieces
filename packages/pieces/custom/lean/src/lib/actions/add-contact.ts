...
import { ADD_CONTACT_OPTIONAL_PARAMS, ADD_CONTACT_REQUIRED_PARAMS } from '../common/constants';
...
export const addContact = createAction({
  ...
  async run(context) {
    ...
    if (login && login?.body?.token) {
      ...
      const endpoint = `${data?.url}/api/v2/customers/people/`;
