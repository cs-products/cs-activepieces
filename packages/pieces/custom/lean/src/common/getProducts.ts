import { httpClient } from "@activepieces/pieces-common";
import { createHttpRequest } from "./createHTTPRequest";

export const getProducts = async (origin: string, hotelId: string, token: string) => {
    const endpoint = `${origin}/api/v2/products?hotel=${hotelId}`;
    try {
        const request = createHttpRequest(endpoint, {}, "GET", token);
        const response: any = await httpClient.sendRequest<{
            Services: any;
        }>(request);
        console.log("response get products",JSON.stringify(response?.body));
        return response;
    } catch (err: any) {
        console.log("Error occured while fetching products!", JSON.stringify(err));
        return {
            status: 500,
            message: "Internal Server Error",
        };
    }
}