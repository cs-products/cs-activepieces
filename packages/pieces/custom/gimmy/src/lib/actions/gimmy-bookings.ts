...
export const gimmyBookings = createAction({
  ...
  async run(context) {
    try {
      ...
      if (!body) {
        throw new Error(`Invalid request7 ${JSON.stringify(body)}`);
      }
      ...
      console.log("logged in", token)
        const url = `https://demo.hotel-data.fr/pds/api/bookings/${hotelId}`;
