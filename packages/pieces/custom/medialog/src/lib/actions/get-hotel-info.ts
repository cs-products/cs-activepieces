...
import {
  createGatewayResponseForCountryCultures,
  createGateWayResponseForHotelInfo,
  createGatewayResponseForSegments,
  createSecretToken1,
  decode,
...
export const getHotelInfo = createAction({
  ...
  async run(context) {
    ...
    if (response?.status === 200) {
      ...
      const segments =
        segmentsResponse?.status == 200
          ? createGatewayResponseForSegments(segmentsResponse)
          : [];
      ...
      const cultureDetailsresponse = await getCountryCultures(
        countryCulturesEndpoint,
        creds['Login'],
        pwd,
        hotelId
      );
      ...
      const hotelCulture =
        cultureDetailsresponse?.status == 200
          ? createGatewayResponseForCountryCultures(cultureDetailsresponse)
          : '';
      console.log('cultures', JSON.stringify(cultureDetailsresponse));
      console.log('flow complete');
      return createGateWayResponseForHotelInfo(
        hotelId,
        response,
        segments,
        hotelCulture
