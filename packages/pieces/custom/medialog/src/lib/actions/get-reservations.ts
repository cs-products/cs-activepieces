...
export const getReservations = createAction({
  ...
  description: 'To retrieve reservations within time range',
  ...
  async run(context) {
    ...
    try {
      ...
      while (condition) {
        ...
        const allArrivingReservations = await getArrivals(
          arrivalsEndpoint,
          creds['Login'],
          password,
          hotelId
        ...
        if (allArrivingReservations.status == 200) {
          const newReservations = transformArrivalDepartureResponse(
            allArrivingReservations
          );
          console.log('new Res', JSON.stringify(Object.keys(newReservations)));
          allReservations = addRservationWhichAreNotPresent(
            newReservations,
            allReservations
          );
        ...
        if (currentDate.getTime() != new Date(dateTo).getTime()) {
          const allDepartingReservations = await getDepartures(
            departuresEndpoint,
            creds['Login'],
            password,
            hotelId
          ...
          if (allDepartingReservations.status == 200) {
            const newReservations = transformArrivalDepartureResponse(
              allDepartingReservations
            );
            console.log(
              'new Res deprtures',
              JSON.stringify(Object.keys(newReservations))
            );
            allReservations = addRservationWhichAreNotPresent(
              newReservations,
              allReservations
        ...
        console.log(
          'allReservations:::',
          JSON.stringify(Object.keys(allReservations).length)
        );
      }

      const reservationDetails =
        createBookingDetailsAccordingToGateway(allReservations);
      return {
        status: 200,
